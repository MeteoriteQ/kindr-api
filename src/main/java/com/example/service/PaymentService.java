...
public class PaymentService {
    ...
    @Transactional
    public Map<String,Object> createOrder(User u,CreatePaymentRequest r,String guest) throws Exception {
        ...
        Payment p=new Payment();p.setId(Ids.id("pay"));p.setDonorId(donorIdentity(u,guest));p.setCampaignId(c.getId());p.setAmount(amount);p.setAnonymous(u==null || r.isAnonymous());p.setMessage(trim300(r.getMessage()));p.setMethod(r.getMethod()==null?"card":r.getMethod());p.setStatus("Pending");p.setMode(live()?"razorpay":"test");
        if(live()){
    ...
    @Transactional
    public Map<String,Object> confirm(User u,ConfirmPaymentRequest r,String guest) throws Exception {
        Payment p=payments.findByIdForUpdate(r.getPaymentId()).orElseThrow(()->new NoSuchElementException("Payment not found."));
        if(!Objects.equals(p.getDonorId(),donorIdentity(u,guest)))throw new SecurityException("This payment does not belong to you.");
        ...
        Donation d=new Donation();d.setId(Ids.id("don"));d.setPaymentId(p.getId());d.setProviderPaymentId(p.getProviderPaymentId());d.setDonorId(donorIdentity(u,guest));d.setCampaignId(c.getId());d.setCampaignTitle(c.getTitle());d.setAmount(p.getAmount());d.setAnonymous(p.isAnonymous());d.setMessage(p.getMessage());d.setStatus("Completed");donations.save(d);
    ...
    private static String donorIdentity(User u,String guest){if(u!=null)return String.valueOf(u.getId());if(guest==null||guest.isBlank())throw new SecurityException("Guest checkout session expired.");return "guest:"+guest;}
